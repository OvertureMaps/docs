...
Based on feedback from developers and data analysts in the Overture ecosystem, we simplified ("flattened") the structure of our road data to make it easier to query road attributes. TO=o make this transition easier for users, we also included the now deprecated road JSON column in this release. 
...
:::warning
- Breaking change: transformed `segment` properties from JSON to structured Parquet columns
