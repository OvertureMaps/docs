...
Overture's `2024-07-22` release is live. Our base, buildings, divisions, and places themes are now in General Availability (GA). Our transportation theme is still in beta, and the Overture transportation task force anticipates breaking changes in the next release. We also released a new addresses theme, available in alpha. The Overture addresses task force will be making significant changes to the schema and updating and adding to the data over the next several releases. 

The datasets are available in GeoParquet and stored on AWS and Azure. The release paths are:
...
In this release, we added a new theme called addresses that includes over 200 million address features for 14 countries. We are releasing this theme in alpha and anticipate significant changes over the next several Overture releases. We invite the Overture community to test the addresses schema and data. User will inform the schema evolution and help us ensure it will work across a global addresses dataset.

### GERS changelog

We are now publishing a GERS changelog with each release that captures feature changes by Overture ID. The changelog is available as Parquet files &mdash; partitioned by theme, type, and `change_type` &mdash; at the following locations:
...
We removed `update_time` from a top-level schema property to a property within the sources structure. See below for an example of this change in the buildings theme.

```
Microsoft buildings `update_time` comes from Microsoft's imagery capture data.
...
**Addresses**
- added over 200 million addresses for 14 countries in this brand new Overture theme
