...
Overture is now releasing bridge files alongside our monthly data releases. Bridge files map GERS IDs to source data IDs, providing a comprehensive matrix that traces the origin of all data in the Overture corpus, including data that doesn't appear in our published releases.
...
In next month's release (late June), Overture is changing the format of its [`id` property](/schema/#top-level-properties) to standard [UUID](https://www.rfc-editor.org/rfc/rfc9562.html) strings of hexadecimal characters, separated by dashes: 
```
xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```

Currently the structure of our IDs varies slightly by `theme`, with some themes using encoded data structures (e.g. [H3](https://h3geo.org/)). The new UUIDs will be randomly-generated identifiers with no additional data that can be extracted or interpreted beyond their identity function. Moving to UUIDs enables centralized ID generation and registration across our pipelines, which is necessary for successful implementation of [GERS](/gers). We will support our users by providing a one-time file that maps the old IDs to the new UUIDs.
