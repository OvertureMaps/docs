...
1. To download places data for a specific region, we can use the following DuckDB query to download the places data within a specific bounding box and generate a `GeoJSON` file.
    ...
    COPY (
        ...
        FROM read_parquet('s3://overturemaps-us-west-2/release/2023-10-19-alpha.0/theme=places/type=*/*',
...
1. Run the following DuckDB to download placenames from the admins theme for the bounding box to create `placenames.geojson`
    ...
    COPY (
        ...
        FROM read_parquet('azure://release/2023-10-19-alpha.0/theme=admins/type=locality/*',
                            filename=true, hive_partitioning=1)
...
Since there are 1.3B buildings in the Overture data, we can first leverage Amazon Athena to filter for only the buildings in the bounding box.

1. Run this query in  Amazon Athena against the `overture_2023_10_19_alpha_0` table, as set up in [these instructions](https://github.com/OvertureMaps/data#accessing-overture-maps-data-1). Download the results as `buildings.csv`
...
3. Run [tippecanoe](https://github.com/felt/tippecanoe) to produce a `pmtiles` archive from `buildings.geojsonseq`
    ...
    ```bash
    tippecanoe -o buildings.pmtiles -Z13 -z13 -l buildings -P buildings.geojsonseq
    ```
...
The map on this page combines a [react maplibre example](https://docs.maptiler.com/react/maplibre-gl-js/how-to-use-maplibre-gl-js/) with [PMTiles](https://codesandbox.io/s/black-dream-oycvf2?file=/src/App.js). You can [view the source](https://github.com/OvertureMaps/how-to/blob/building-a-map-improvements/src/components/map.js) for the above map on github for a complete example using react, or, check out the [map based on July's release](https://labs.overturemaps.org/overture-with-daylight) for a non-react implementation of PMTiles.

Using PMTiles with react requires adding the [PMtiles protocol](https://docs.protomaps.com/pmtiles/maplibre), like this in react:
...
Listed in each tab are the complete styles in the [Maplibre Style Spec](https://maplibre.org/maplibre-style-spec/) for the map on this page.
