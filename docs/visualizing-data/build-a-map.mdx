...
Overture data is released in the cloud-native format `geoparquet`. It is partitioned by theme. Accessing the data through a service that can take advantage of these properties provides the best performance. This example will use [DuckDB](https://duckdb.org/). After downloading and converting the data to GeoJSON, we can use [tippecanoe](https://github.com/felt/tippecanoe) to create a [`PMTiles`](https://github.com/protomaps/PMTiles) archive for each theme.
...
Overture contains billions of buildings across dozens of geoparquet files. The following query will scan the metadata for each file and download only the buildings within our bounding box.

1. This DuckDB query downloads buildings from Overture to a `GeoJSONSeq` file.
