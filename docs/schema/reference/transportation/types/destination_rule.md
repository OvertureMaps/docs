...
| `labels[]` | `list<`[`DestinationLabels`](destination_labels.md)`>` (optional) | Labeled destinations that can be reached by following the segment.<br/>*At least one of `labels`, `symbols` must be set* |
...
| `symbols` | `list<`[`DestinationSignSymbol`](destination_sign_symbol.md)`>` (optional) | A collection of symbols or icons present on the sign next to current destination label.<br/>*At least one of `labels`, `symbols` must be set* |
