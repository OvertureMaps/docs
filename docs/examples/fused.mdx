...
By encapsulating the code into a UDF, Fused UDF builder can dynamically pass the bbox run and show on map to perform the join on the fly. 

While you don't have to use it, the fastest way to visualize the output and iterate on the code is to run this UDF in [the Fused workbench](https://www.fused.io/workbench/catalog/Overture_Nsi-dd89972c-ce30-4544-ba0f-81fc09f5bbef), Fused's browser based serverless IDE. 
...
- Browse the [Fused UDF Catalog](https://www.fused.io/workbench/catalog) to find other examples of reusbale code
