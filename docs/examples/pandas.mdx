...
In this example, we'll demonstrate using using [Pandas](http://pandas.pydata.org/) and [GeoPandas](https://geopandas.org/en/stable/index.html) to handle and manipulate Overture data.


## Requirements
To run this example in a notebook, you'll need to install the following libraries and tools. The full notebook example is available here.
...
GeoPandas
Shapely
DuckDB and DuckDB spatial
jupysql (optional)
duckdb-engine (optional)
```

First, let's import our toolkit. using [Pandas](http://pandas.pydata.org/), a Python package for in-memory data manipulation, and [GeoPandas](https://geopandas.org/en/stable/index.html), a Python library that extends Pandas to allow spatial operations on geometric types. 
 Shapely will help us handle the geometries, and DuckDB is the tool we'll use to fetch only the data we need from Overture's GeoParquet files stored in an S3 bucket.
