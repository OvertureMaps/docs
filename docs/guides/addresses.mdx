...
- **Standardization**: Address standardization is parsing an input address into address components based on an existing schema or address model.
- **Normalization**: Address normalization is adhering to standard and consistent forms of address components.
- **Validation**: Address validation is confirming an address exists within a known list of addresses.
- **Verification**: Address verification is conceptually the same as address validation, although address validation could be expected to work with the most complete roster of addresses (best available source(s)).
