...
import ConnectorBBOX from '!!raw-loader!@site/src/queries/duckdb/connector_bbox.sql';
...
| connector_ids | VARCHAR[] | List of connector IDs identifying the connectors this segment is physically connected to. Each connector is a possible routing decision point, meaning it defines a place along the segment in which there is possibility to transition to other segments which share the same connector. |
...
<details>
    <summary>Class names and feature counts</summary>
<div>
<Tabs>
<TabItem value="classes" label="classes" default>
| subtype | class | Feature count, July 2024 release |
|---|---|---|
| rail |  | 1,891,720 |
| road | alley | 1,507,160 |
| road | bridleway | 94,519 |
| road | crosswalk | 1,736,678 |
| road | cycleway | 1,213,800 |
| road | driveway | 14,722,896 |
| road | footway | 14,149,673 |
| road | living_street | 2,960,512 |
| road | motorway | 1,047,400 |
| road | parking_aisle | 5,860,995 |
| road | path | 12,487,665 |
| road | pedestrian | 445,687 |
| road | primary | 6,917,142 |
| road | residential | 123,084,186 |
| road | secondary | 10,719,092 |
| road | sidewalk | 2,786,910 |
| road | steps | 1,643,010 |
| road | tertiary | 19,288,033 |
| road | track | 23,592,666 |
| road | trunk | 3,843,984 |
| road | unclassified | 28,552,184 |
| road | unknown | 31,725,341 |
| water |  | 27,169 |
</TabItem>
</Tabs>
</div>
...
```
overturemaps download --bbox=12.46,41.89,12.48,41.91 -f geojson --type=segment -o rome_connectors.geojson
``` 
</TabItem>
...
<QueryBuilder query={ConnectorBBOX}></QueryBuilder>
</TabItem>
</Tabs>
...
|:--:| 
| *Conceptual diagram of the splitter tool output. The numbers follwing 1234@ represent start_lr and end_lr values.* | 

The [transportation-spitter tool](https://github.com/OvertureMaps/transportation-splitter) transforms Overture road data into simpler sub segments. It will divide features at each connector point and at each change of a [scoped based property](/schema/concepts/scoping-rules/). Depending on your needs and map stack, the resulting dataset may be easier to manipulate than the original Overture data as each segment will only have connections at either end and have one set of properties for its entire length.
