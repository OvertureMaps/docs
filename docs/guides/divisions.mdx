...
The Overture divisions theme has three feature types &mdash; `division`, `division_area`, and `division_boundary` &mdash; and contains more than 5.3 million point, line, and polygon representations of human settlements, such as such as countries, regions, states, cities, and even neighborhoods. The theme is derived from a conflation of [OpenStreetMap](https://www.openstreetmap.org/) data and [geoBoundaries](https://www.geoboundaries.org/) data and is available under an [ODbL](https://opendatacommons.org/licenses/odbl/) license. 

There are many use cases for the Overture divisions datasets, including but not limited to: reverse-geocoding, cartographic styling of boundaries, place labeling, and point-in-polygon analysis. 
...
<details>
	...
	1. [Feature type descriptions](#feature_types)
	2. [Subtype descriptions](#subtypes)
	3. [Data columns](#data_fields)
</details>

<details>
	<summary>Data access and retrieval</summary>
	1. [DuckDB](#duckdb)
	2. [Python](#python_command-line_tool)
</details>
...
<details>
   <summary>Additional resources and information</summary>
...
### Feature type descriptions <a name="feature_types"></a>

- A `division` is a feature type that represents an official or non-official organization of people &mdash; country, region, province, city, neighborhood, etc. &mdash; as seen from a given political perspective. It has a `Point` geometry which gives an approximate location of the position most commonly associated with the feature.
...
### Subtype descriptions <a name="subtypes"></a>
...
### Data columns <a name="data_fields"></a>
...
[The Getting Data section of this documentation](https://docs.overturemaps.org/getting-data/) offers instructions for using several tools to access Overture data, including DuckDB and Overture's Python command-line tool.

### DuckDB <a name="duckdb"></a>

Using these [DuckDB](https://duckdb.org/) queries, you can get feature counts for each feature type in divisions.

##### Find counts of divisions features
...
<!---
This doesn't seem like the right place for this block of text:
In the divisions dataset, each feature is maintained using a `subtype`. You can find an enumerated list here. You can further iterate on the queries above to find the number of features per divisions theme, or the number of subtype features per country.-->
...
##### Find counts, per subtype, in the `division_area` feature type in the US, Mexico, and Canada
...
##### Find the corresponding Overture feature, given an OSM ID (Relation)

<div>
<QueryBuilder query={OSMLookUp}></QueryBuilder>
</div>

##### View info of `division_boundary` feature types with a bounding box
...
### Python command-line tool<a name="#python_command-line_tool"></a>
...
- [OpenStreetMap](https://openstreetmap.org/). License: [ODbL-1.0](https://www.openstreetmap.org/copyright).
- [geoBoundaries](https://geoboundaries.org/). License: [CC BY 4.0](https://www.geoboundaries.org/#tabs1-js).

You can find a full list of sources and additional licensing information on the [attribution page](https://docs.overturemaps.org/attribution/) in our documentation. 

### Pipeline migration to Overture <a name="pipeline_migration_to_overture"></a>

In late 2024, the Overture Maps Foundation is planning to migrate the conflation logic and pipeline code for divisions from Microsoft's internal infrastructure to Overture infrastructure.
