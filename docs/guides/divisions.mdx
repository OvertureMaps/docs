...
| -------- | -------- | ------- |
| **id** | Varchar	Feature ID. This may be an ID associated with the Global Entity Reference System (GERS) if—and-only-if the feature represents an entity that is part of GERS.
| **geometry** | Geography	Geometry for the administrative boundary
| **bbox** | Variant	Area defined by two longitudes and two latitudes, where: Latitude is a decimal number between -90.0 and 90.0. Longitude is a decimal number between -180.0 and 180.0
| **version** | Number	Version number of the feature, incremented in each Overture release where the geometry or attributes of this feature changed
| **update_time** | Varchar	Timestamp when the feature was last updated
| **sources** | Variant	The array of source information for the properties of a given feature, with each entry being a source object which lists the property in JSON Pointer notation and the dataset that specific value came from. All features must have a root level source which is the default source if a specific property's source is not specified
| **subtype** | Varchar	Subtype of admin area
| **wikidata** | Varchar	A wikidata ID if available, as found on https://www.wikidata.org/.
| **population** | Number	Population in the locality.
| **names** | Variant	The used name when referring to a feature. The first entry in the array of names must have a language of "local" making it the easiest, default name to use among all options
| **class** | 
| **divisions** | 
| **is_disputed** | 
| **perspectives** | 
...
| **region** | Varchar	ISO-3166-2 Country subdivision code.
| **hierarchies** | 
| **parent_division_id** | 
| **norms** | 
