...
The Overture Maps `transportation` theme captures the physical shape and connectivity
...
The [connector](https://docs.overturemaps.org/schema/reference/transportation/connector) feature type carries point geometry which describes a location where a physical connection between two or more segments occurs (or may occur in the
future).
...
The [segment](https://docs.overturemaps.org/schema/reference/transportation/segment) feature type carries `LineString` geometry which describes the physical shape of a section of the transportation network. A segment may represent an entity with a
...
All segments in official Overture Maps transportation data releases have a
...
🚧 We are developing a segment-level directionality concept similar to [lane directionality](/guides/transportation/roads#lane-directionality) to indicate what travel headings are allowed or prohibited along the segment. This effort is
...
cycling and similar activities. For more information about `road` segments, see the page on [roads](/guides/transportation/roads).
...
A primary goal of Overture Maps' segmentation process is to promote stability of
segment shape across Overture Maps data releases. For example, if a certain
...
Overture Maps aims for a stable transportation dataset. On the contrary, we aim to
...
The reason Overture Maps pursues shape stability is to improve the ability to assess whether two segments from different points in time (or from different upstream data
sources) represent the same real-world entity. Overture Maps' success at this assessment directly feeds into the stability and precision of [Global Entity Reference System (GERS)](/guides/gers) IDs assigned to segments. In turn, higher GERS ID stability and precision makes transportation theme data more useful for conflation.

### Interior connectors

A key feature of the Overture Maps transportation schema which enables shape
...
internal connectors on the north-south road segments bordering the block to the east and west. As a result, the [GERS](/guides/gers) IDs of the north-south road segments remain as they were and no data needs to be re-conflated.
...
Note that in the above example, an official Overture Maps data release would insert
coordinates in the middle of the north-south segments, if they did not already
exist, because Overture Maps data releases will always ensure that every segment's
...
property values as being geometrically scoped and discuss geometric scoping at greater length in the page on [scoped properties](/guides/feature-model/scoping-rules).
...
stability, ultimately resulting in more precise and stable [GERS](/guides/gers) IDs and less churn in conflated data.
...
Although it is technically possible to use the Overture Maps schema to express a
