...
[segment](//docs.overturemaps.org/schema/reference/transportation/segment) features with the `subtype` property value set to the value `road`.
...
for the [travel mode](/guides/transportation/travel-modes) `foot` and deny access to all other
...
[geometric scoping](/guides/feature-model/scoping-rules#geometric-scoping-linear-referencing)
...
[rules](/guides/feature-model/scoping-rules#rules-and-rule-based-properties) will be used to
...
[physically connected](/guides/transportation/shape-connectivity#physical-connectivity)
segments.
...
the `forward` [heading](/guides/transportation/shape-connectivity#heading), then it is implied
...
specified using [rules](/guides/feature-model/scoping-rules#rules-and-rule-based-properties).
...
<!--
    FIXME: I found this dangling link on 2024-03-11. Once fixed, this
           comment can be removed and the text can be replaced in its
           former state. 👇

    Many transportation [use cases](/themes/transportation#use-cases) can be
-->

Many transportation use cases can be
...
being modeled, facing in the [direction](/guides/transportation/shape-connectivity#directionality)
...
[orientation of the segment](/guides/transportation/shape-connectivity#start-end-and-orientation).
...
`both_ways`) should be used along with
[Temporal Scoping](/guides/feature-model/scoping-rules#temporal-scoping-opening-hours) at the
lane block level.
...
Consequently, instead of having a static lane block, road segments carry a list
of lane block [rules](/guides/feature-model/scoping-rules#rules-and-rule-based-properties)
...
- A rule may also be scoped [temporally](/guides/feature-model/scoping-rules#temporal-scoping-opening-hours),
which allows the time of day that the lane block exists to be specified.
Temporal scoping is useful for modeling cases like lanes which are used
for parking at off hours and traffic during peak hours. In such a case,
the lane block rule would be scoped to peak hours.

As with all rule-based properties in the Overture schema, the
[rule evaluation algorithm](/guides/feature-model/scoping-rules#rule-evaluation-algorithm)
must be applied to determine which lane block rule, if any, is
...
- The blue shaded, or southwesterly, segment is
  [oriented](/guides/transportation/shape-connectivity#directionality) toward
  the north-east. It has two geometrically-scoped lane block rules:
...
*A segment with two [geometrically-scoped](/guides/feature-model/scoping-rules#geometric-scoping-linear-referencing)
