...
[segment](//docs.overturemaps.org/schema/reference/transportation/segment) feature.
...
use the segment. For [road](/guides/transportation/roads) segments, this implied set derives
from the [road class](/guides/transportation/roads#road-class), as well as local rules, norms,
...
- the proposed new travel mode should preferably not be trivially
   ...
   [scoping property](/guides/feature-model/scoping-rules#scoped-values-and-scoping-properties).
...
access restrictions, turn restrictions, or speed limits on [road](/guides/transportation/roads)
segments. Since travel modes are expressed via scoping properties, it
is recommended to read the section on
[scoped and rule-based properties](/guides/feature-model/scoping-rules) before proceeding further.
...
[vehicle attributes scoping](/guides/feature-model/scoping-rules#vehicle-attributes-scoping). Where there is potential overlap, the method of scoping used in Overture
...
- The `using` property indicates
  [purpose of use](/guides/feature-model/scoping-rules#purpose-of-use-scoping) scoping, which
...
- The `recognized` property indicates
  [status scoping](/guides/feature-model/scoping-rules#status-scoping-membership-in-a-recognized-group),
