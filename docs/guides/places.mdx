...
This will download the area visible in on your screen.
</TabItem>
<TabItem value="Python Command-line Tool" label="Python Command-line Tool" default>
First, follow the [setup guide for the Python Command-line Tool](/getting-data/overturemaps-py/).
...
Overture Places can be a valuable source for conflating with or enhancing in your own existing dataset. 
...
Using some basic matching logic we can join these two datasets together to create a more robust final product. By also joining the GERS id to our output dataset we could easily update our now conflated dataset to future Overture releases with a simple join.

To run this example yourself, the latest Oregon PBF can be obtained from [Geofabrik](https://download.geofabrik.de/north-america/us/oregon.html).
<details>
