...
Now each release is available as a table under the `release` database in the `overture` data source (note the `v` at the beginning of the table name):
