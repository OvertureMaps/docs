...
import MadridBuildings from '!!raw-loader!@site/src/queries/duckdb/buildings_madrid.sql';
...
<QueryBuilder
  query="s3://overturemaps-us-west-2/release/__OVERTURE_RELEASE/theme=base/type=infrastructure/*.parquet"
  language="text"
></QueryBuilder>
...
<QueryBuilder
  query="aws s3 cp --no-sign-request --recursive s3://overturemaps-us-west-2/release/__OVERTURE_RELEASE/theme=base/type=infrastructure/ ."
  language="text"
></QueryBuilder>
...
<QueryBuilder
  query="aws s3 cp --no-sign-request --recursive s3://overturemaps-us-west-2/release/__OVERTURE_RELEASE/theme=buildings/type=building/ ."
  language="text"
></QueryBuilder>
...
<QueryBuilder
  query='azcopy copy "https://overturemapswestus2.dfs.core.windows.net/release/__OVERTURE_RELEASE/theme=places/type=place/" "<<local directory path>>"  --recursive'
  language="text"
></QueryBuilder>
...
[DuckDB](https://duckdb.org/) is one of our favorite tools for querying the data and downloading only what you need. We have a full page of example queries [here](/getting-data/duckdb/); we'll give you one quick example below.

Let's say you don't want the entire buildings dataset for the planet. With DuckDB, you can scan Parquet files directly in S3 and extract only the data that matches specific parameters:

<QueryBuilder query={MadridBuildings}></QueryBuilder>
...
In addition to the official sources maintained by the Overture Maps Foundation, several _friends of Overture_ are maintaining mirrors of the data on these platforms:
