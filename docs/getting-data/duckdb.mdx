...
You can also use DuckDB to loop over the Parquet files in a directory and convert the data to a spatial format of your choice. The spatial extension uses GDAL so you could export to PBF if that's the data format that works best for you. Here's a bash script that shows you how:
