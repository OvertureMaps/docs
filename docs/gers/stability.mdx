...
## History matching, and why your crosswalk mirrors it
...
## Two events that broke stability by design

**The June 2025 identifier switchover.** Overture replaced every GERS ID with a UUID in the `2025-06-25.0` release. Churn was 100 percent by design, and a one-off mapping from old IDs to new ones was published alongside that release. If you are carrying IDs from before June 2025 and have not migrated, your IDs will not resolve against any current release. [PLACEHOLDER: confirm whether the old-to-new mapping file is still available, and at what path. Two internal paths are recorded, `s3://overturemaps-extras-us-west-2/june_to_may_id_mapping/` and an internal bucket, and the June 2025 release notes described the mapping as available "for this release only."]
...
[PLACEHOLDER: Overture tracks internal churn targets per theme, currently 5 percent for places and better than 99 percent stability for transportation. Decide whether to publish these. My recommendation is yes, with the release they were measured against and a note that they are targets rather than guarantees, because evaluators who cannot find a number assume the worst. A team ingesting the addresses theme concluded from their own measurements that churn was roughly 20 percent and wrote a standing rule that nothing in their schema may join on a GERS ID. They arrived at that by measuring, because we published nothing.]
