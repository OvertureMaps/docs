...
Depending on the feature type, unique IDs registered to GERS are intended to be [stable](stability) &mdash; within a reasonable tolerance of error &mdash; across multiple Overture releases. When stability is not possible, we will attempt to provide traceability in a [GERS changelog](changelog). For example:
