...
GERS IDs are 32 characters and 128 bits. For most features types, the GERS ID is intended to be [stable](stability) across [Overture releases](/releases). This means a GERS ID assigned to a `building` feature, for example, won't change even if the building's properties change, including changes to its geometry.
...
As many users have noticed, the GERS IDs in some Overture datasets contain an [H3 index](https://h3geo.org). Going forward, Overture cannot guarantee that a feature with an H3 component to its GERS ID will be permanently located in the expected H3 cell. As we cannot support H3-dependent pipelines, we encourage you to use `bbox` instead. 
:::
