...
```
We can see that it's a `place` type that was first seen by the [GERS Registry](/gers/registry) in June 2025. This makes sense because the GERS Registry didn't exist to _see_ any features before that. 
</details>
...
Every Overture release (except the June 2025 release) includes a [changelog](/gers/changelog) with a high level overview of data added, removed, or changed, based on the ID. The changelog is partitioned by `theme`, `type`, and `change_type`. Let's use the changelog to identify all of the features added in Salt Lake City in Overture's May release.
...
[Bridge files](/gers/bridge-files) are published mappings of GERS ID to Source IDs for Overture features that came from an established dataset with a meaningful `record_id`. ML-Derived buildings, for example, do not have stable meaningful input IDs, but place records from Meta have corresponding IDs that reference public Facebook pages.
