...
Two weeks ago we [quietly published v2.0.0](https://github.com/OvertureMaps/schema/pull/709) of the Overture schema to [PyPI](https://pypi.org/project/overture-schema/). It used to be JSON Schema, published monthly alongside each [data release](https://docs.overturemaps.org/blog/tags/releases/) as [handwritten YAML files](https://github.com/OvertureMaps/schema/tree/main/schema). Now it’s a Python library you install and import, with Pydantic models you can validate data against, ours or your own. 

For most of you – our data enjoyers and schema readers – this is not a big deal. You might notice that we made a few correctness fixes to the schema structure and improved our documentation.

For others, this is a huge and welcome change. The schema has gone from a document to code that is useful to you, independent of the data itself. 

In any case, you can do this now:
...
You can ask a feature type what fields it has, read the documentation for anything, and validate a record against a feature type or other model. You can do that from the Python interpreter, your favorite IDE with type hints and completion, or at scale within a Spark job. 
...
We began porting the schema from JSON Schema to Pydantic last August, and since last November we've updated both the Pydantic models and the YAML versions with every release. The new Pydantic models are now the source of truth, and everything else we ship is generated from them: the [JSON Schema](https://github.com/OvertureMaps/schema/releases/download/overture-schema-v2.0.0/overture-schema.json), the [PySpark validation expressions](https://github.com/OvertureMaps/schema/tree/main/packages/overture-schema-pyspark) used by Overture’s internal data pipelines, and the [reference documentation](https://docs.overturemaps.org/schema/) on this site. They can't drift apart, because they all come from the same place.
...
The [YAML files](https://github.com/OvertureMaps/schema/tree/main/schema) are deprecated as of this release, and we'll remove them in December 2026. The JSON Schema stays, except now it’s a derived output instead of the source of truth. It’s functionally the same, even if it’s structured differently and published as JSON. You can grab it as a [release artifact](https://github.com/OvertureMaps/schema/releases/download/overture-schema-v2.0.0/overture-schema.json), or generate it yourself for the subset of types you need:
...
You can already use the new libraries to build a custom feature type that stands on its own. What you can't do yet is add a field to one of ours. That’s coming soon.

Our next milestone is schema extensions. An extension standardizes a set of schema fields -- opening hours for POIs, capacity for buildings, advisory speeds for roads --  and will enable an ecosystem of new data that interoperates with Overture without needing to be hosted by Overture. Data that's proprietary, or that refreshes far faster than our release cadence, can't live inside an Overture release; this way it doesn't have to. An extension adds columns to features that already carry a [Global Entity Reference System](https://docs.overturemaps.org/gers/) (GERS) ID, so the consumer can connect the extension data to the Overture data by joining on the GERS ID.

## Read more

The schema reference documents every feature type property by property, and the theme guides explain what the data means before you write a query against it. The schema repo carries the working documentation for the models: how to install and use the packages, how to author using them, the conventions we follow, and the vocabulary we use for all of it.

## Talk to us

Talk to us

We build in the open at github.com/OvertureMaps/schema. File an issue, open a discussion, or write to us at community@overturemaps.org. Tell us what breaks or what you've built on top of our new foundation. And if you have data you'd attach to Overture features, let us know how we can support you.
