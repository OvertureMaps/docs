...
**The `2026-02-18.0` release of Overture data and `v1.16.0` of the Overture schema are now available.** The datasets, changelog, GERS registry, bridge files, and PMTiles are available on both AWS and Azure. In this release we added a new `admin_level` property to the Divisions schema to represent a division’s position in its country’s hierarchy. We also updated our places guide with more information about our new taxonomy, including mappings across `basic_category`, `taxonomy`, and `categories`.
...
- added new Tom-Tom sourced road segments in over 65 new countries including Azerbaijan, Sri Lanka, Nigeria, and El Salvador.
- added over 1.6M new Tom-Tom sourced road segments with a combined length of over 280k km compared to January.
