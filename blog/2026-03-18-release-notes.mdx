...
The `bbox` column fields (`bbox.xmin`, `bbox.ymin`, `bbox.xmax`, `bbox.ymax`) have changed to [Parquet Double](https://parquet.apache.org/docs/file-format/types/) instead of 32-bit Floats. This change works around an outstanding Athena bug affecting queries on nested float fields.
