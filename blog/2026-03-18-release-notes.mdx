...
The `bbox` column fields (`bbox.xmin`, `bbox.ymin`, `bbox.xmax`, `bbox.ymax`) have changed to the [Parquet physical type `DOUBLE` (64-bit floating point)](https://parquet.apache.org/docs/file-format/types/) instead of 32-bit `FLOAT`. This change works around an outstanding Athena bug affecting queries on nested float fields.
...
- adjusted methods of calculating feature-level confidence scores
