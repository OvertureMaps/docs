...
In the places theme, we're added a new `operating_status` property, with all values set to `open`. In future releases, we plan to update this property with data that indicates whether a place is `open`, `permanently_closed`, or `temporarily_closed`.

## Signal "patches"
About a million POIs in our Places theme now have a signal "patch" that affects the calculation of the score in the `confidence` property. POIs enhanced by these patches have `SparkGeo-confidence-conflation` in the `sources` property.
