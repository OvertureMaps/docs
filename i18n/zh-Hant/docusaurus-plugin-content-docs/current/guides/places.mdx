...
本指南專注於 Overture places **數據** &mdash; 其內容、範圍、屬性和使用案例。若要解更多關於 Overture places **架構** 的相關資料，請參見 [架構參考文檔](/schema/reference/places/place)。

:::

## 概要

Overture places 主題有一個特徵類型，稱為 `place`，包含超過 5300 萬個真實世界實體的點表示：學校、商業、醫院、宗教組織、地標、山峰等。該主題源於 Meta 和 Microsoft 數據的合併，並在 [CDLA Permissive 2.0](https://cdla.dev/permissive-2-0/) 授權下提供。

| ![Overture places 主題覆蓋範圍](/img/places/places-coverage.png) | 
|:--:| 
| *Overture places 數據，按數據來源樣式：Meta 用紫色，Microsoft 用橙色。* | 
