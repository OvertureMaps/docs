...
用新的 bounding box 替換 `bbox.xmin` 和 `bbox.ymin` 值，以針對不同區域運行查詢。
...
這些示例使用地址、類別和信心分數列中的資料屬性，以便通過 DuckDB 以有用的方式過濾資料。
<Tabs>
...
要自己運行此範例，可以從 [Geofabrik](https://download.geofabrik.de/north-america/us/oregon.html) 獲取 Oregon 的 PBF 檔案。
