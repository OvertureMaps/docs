...
根據 AWS 用戶帳戶/角色所擁有的權限，可能需要明確啟用對 Overture 目錄的訪問。可以通過 [添加新的 IAM 策略](https://console.aws.amazon.com/iam/home?#/policies?type=customer) &mdash; 建議將其命名為 `OvertureGlueCatalogAccess` &mdash; 並設定以下權限：
...
如果您更願意在自己的資料目錄中創建表，您可以在 Athena 查詢控制台中運行完整的 DDL 查詢。這將在您的 AWS 帳戶資料目錄中創建表格，直接指向托管在 Overture 的 S3 儲存貯體上的資料。
...
在運行 set-up 查詢後，可以通過執行 `MSCK REPAIR TABLE overture` 或從表格選項選單中選擇「Load Partitions」來加載分區。

### 在您的資料目錄設定中添加策略

如果您想與其他用戶和帳戶共用此目錄，需要添加一個策略以啟用跨帳戶 Glue 目錄訪問。為此，請到 [資料目錄設定](https://console.aws.amazon.com/glue/home#/v2/security/settings)，並添加一個類似於以下內容的目錄策略：
