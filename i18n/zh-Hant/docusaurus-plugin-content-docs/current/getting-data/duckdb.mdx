...
[在本地安裝 DuckDB](https://duckdb.org/docs/installation/)。您將需要擴充套件功能來處理雲端的空間數據。使用 DuckDB CLI，請執行以下操作：
...
### 山脈
此查詢從 Overture 地點數據集中選擇山脈類別的 POI，並將其輸出到 GeoJSON 中。
...
此查詢從 Overture 建築數據集中抓底特律地區的建築幾何形狀和選定屬性。

<QueryBuilder query={DetroitBuildings}></QueryBuilder>

### 行政區輪廓

此查詢從 divisions 數據集中抓賓夕法尼亞州的縣的幾何形狀，並將其輸出到 GeoJSON 中。
