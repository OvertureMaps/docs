...
時間範圍限定結構 `when: { during: "..." }` 將其父值的範圍限制為一個或多個重複的時間區間。當父值是規則對象時，該規則僅匹配 `during` 屬性中指定的時間區間或時間區間集合。
...
下面的範例顯示了一個具有時間範圍限定的道路段。該規則指出，非公車旅客在平日的下午3點到6點之間禁止進入該段道路。
...
主觀範圍限定意味著屬性的範圍可以根據主觀因素來限制，例如*誰*或*什麼*正在使用交通網路，或*如何*使用。
...
方向範圍限定將父值的範圍限制為僅在旅行者沿著段幾何形狀向指定方向（`forward` 或 `backward`）行進時適用。 (*方向 `forward` 和 `backward` 的定義見於 [形狀和連接性](https://docs.overturemaps.org/guides/transportation/shape-connectivity) 頁面。*)

屬性構造 `when: { heading: forward|backward }` 將方向範圍限定應用於父值。


  </TabItem>
  <TabItem value="example" label="Example" default>

下面的例子顯示了一個擁有多個方向範圍限定的道路段。這些規則允許所有標準旅行模式在段的正向方向上行駛，但僅允許巴士在反向方向上行駛。
