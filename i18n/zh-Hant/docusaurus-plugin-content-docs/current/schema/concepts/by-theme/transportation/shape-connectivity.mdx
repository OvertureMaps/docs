...
Overture 運輸主題通過主題中的兩種要素類型：段落和 Connector 之間的互動，捕捉運輸網路的物理形狀和連接性。
...
[ Connector ](/schema/reference/transportation/connector)要素類型攜帶點幾何，描述了兩個或多個段落之間發生（或未來可能發生）物理連接的位置。

 Connector 除了幾何和標準的Overture要素屬性之外，沒有其他屬性。運輸主題的所有其他方面都建模於段落。

<!--
 技術上來說， Connector 沒有其他屬性的說法是不正確的，因為截至2023-06-15，它們有一個 `level` 屬性，可能應該被移除：https://github.com/OvertureMaps/schema-wg/issues/138
-->
...
如果每個段落的 Connector 屬性包含對 Connector 的引用，則兩個或多個段落在給定的 Connector 處物理連接。

 Connector 幾何的座標應該盡可能包含在段落幾何的座標內，在這種情況下， Connector 座標定義了物理連接點。官方的Overture資料發布將始終滿足此約束。如果無法滿足，則物理連接點是與 Connector 座標相交的段落幾何上最近的點。

相反，如果兩個段落的 Connector 屬性未引用共享 Connector ，即使它們的幾何形狀重疊或共享公共座標，它們也不會被物理連接。

允許從一個段落上的點到另一個物理連接段落上的點的旅行，除非受到明確限制，如進入或轉向限制。

官方的Overture運輸資料發布中的所有段落至少有兩個 Connector ，一個在幾何形狀的每端，即使這些端點 Connector 未附加到其他段落上。這不是強制性最低要求，也不由模式強制執行。這樣做是為了讓新段落能夠連接到現有網路中，而不需要更改現有段落的屬性。
...
請注意，兩個具有不同 `level` 值的段落可能會彼此物理連接，因為 `level` 是渲染的近似值，不是段落不同點的精確高度指示。 Connector 沒有 `level` 屬性。
...
「分段」這個術語描述了將上游源資料轉換為 Overture 運輸形狀和連接資料的過程，這些資料以段落和 Connector 的形式建模。
...
- [內部 Connector ](#interior-connectors)。
...
Overture 運輸模式的一個關鍵 Features 是段落能夠支援在其幾何形狀內部位置的 Connector ，而不僅僅是在其端點。能夠添加內部 Connector 防止分段過程必須盲目跟隨上游源資料中引入的每個切割或合併。

假設一個四面圍繞道路的方形城市街區已經在源資料中映射，但沿東西方向分割街區的後巷尚未映射。如果後巷隨後在源資料中被映射，Overture 分段過程可以連接到運輸網路，而不需要通過簡單地在街區東西側的南北方向路段上引入內部 Connector 來細分任何現有段落。因此，南北方向路段的 [Overture IDs](https://docs.overturemaps.org/gers) 保持不變，且無需重新合併資料。
...
請注意，在上述範例中，官方的 Overture 資料發布會在南北方向段落的中間插入座標（如果它們尚不存在），因為 Overture 資料發布將始終確保每個段落的幾何形狀包括所有 Connector 。從計算機的角度來看，這是對段落形狀的非常輕微的修改。

### 幾何範圍

許多段落屬性可能包含線性參考，使其僅適用於段落幾何的一部分。我們將這些線性參考的屬性值稱為幾何範圍，並在 [範圍屬性](/schema/concepts/scoping-rules) 頁面中對幾何範圍進行更詳細的討論。

幾何範圍允許分段算法避免僅因為某個屬性在幾何形狀的不同部分具有不同值而引入段落分割。像內部 Connector 一樣，幾何範圍屬性使分段過程能夠做出促進形狀穩定性的決策，最終導致更精確和穩定的 [Overture IDs](https://docs.overturemaps.org/gers) 和更少的資料變動。
...
一個非法環路，其中一個段落連接到另一端，可以通過切割段落並引入第二個 Connector 來保持物理連接來進行修正。對於非法的自交環路，度數為 *N*，可以通過將段落切割成 N 個部分來進行修正。
