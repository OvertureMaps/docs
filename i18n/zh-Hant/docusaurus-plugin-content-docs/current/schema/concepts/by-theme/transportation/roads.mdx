...
在 Overture 運輸主題中，道路是指任何類型的道路、街道或路徑，包括專用的步行或自行車道，但不包括鐵路。道路段構成了大部分地面運輸段。道路使用 [segment](/schema/reference/transportation/segment)  Features 來建模，並將 `subtype` 屬性值設定為 `road`。

## 幾何形狀和粒度

道路段的幾何形狀是道路的表示，通常近似於所建模道路段的實際中心線。道路段支援以不同粒度建模道路網路。例如，一個道路段可以表示：
...
道路的 `class` 屬性指定了其一般用途和在交通網路中的相對重要性。`class` 屬性還有助於建立合理的預設值。例如，`sidewalk` 類別意味著該段的預設 [通行限制](#access-restrictions) 允許 `foot`（步行）這一旅行模式的通行，並禁止所有其他旅行模式的通行。與許多道路段屬性不同，道路段的 `class` 屬性不支援 [幾何範圍限定](#geometric-scoping-linear-referencing)（線性參考）。因此，當現實世界的道路在某個線性範圍內發生類別變化（例如，從次級道路變為住宅道路）時，Overture 運輸 [分段](#segmentation) 算法將會生成一個段分割。
